...
class MatcherApp(toga.App):
    ...
    def startup(self):
        ...
        # Split view: left for info+table, right for preview
        split_container = toga.SplitContainer(style=Pack(flex=1))

        # Left side: info and table
        left_box = toga.Box(style=Pack(direction=COLUMN, margin=10, flex=1))

        # Record info box
        info_box = toga.Box(style=Pack(direction=COLUMN, margin_bottom=10))
        self.lbl_voucher   = toga.Label('', style=Pack(margin_bottom=5))
        self.lbl_date      = toga.Label('', style=Pack(margin_bottom=5))
        self.lbl_amount    = toga.Label('', style=Pack(margin_bottom=5))
        self.lbl_creditor  = toga.Label('', style=Pack(margin_bottom=5))
        self.lbl_text      = toga.Label('', style=Pack(margin_bottom=5))
        ...
        left_box.add(self.table)

        # Right side: preview area
        self.preview_scroll = toga.ScrollContainer(style=Pack(flex=1))
        self.preview_scroll.content = toga.Label("Select a document to preview", style=Pack(margin=10))
        right_box = toga.Box(style=Pack(direction=COLUMN, margin=10, flex=1))
        right_box.add(self.preview_scroll)

        split_container.content = [(left_box, 1), (right_box, 1)]
    ...
    def show_record(self):
        ...
        # Clear preview when showing a new record
        self.preview_scroll.content = toga.Label("Select a document to preview", style=Pack(margin=10))

    def show_preview(self, widget, row=None):
        """Show preview of the selected document in the table."""
        # If row is None, try to get the current selection
        if row is None:
        ...
        # Construct full file path using docs_dir
        file_name = row.file
        path = self.docs_dir / file_name
        if not path.exists():
            self.preview_scroll.content = toga.Label(f"File not found: {file_name}", style=Pack(margin=10))
            return
        ...
        try:
            if ext in (".jpg", ".jpeg", ".png"):
                ...
                self.preview_scroll.content = toga.ImageView(img, style=Pack(flex=1))
            elif ext == ".pdf":
                ...
                if images:
                    ...
                    self.preview_scroll.content = toga.Label("Cannot preview empty PDF", style=Pack(margin=10))
            else:
                self.preview_scroll.content = toga.Label(f"Unsupported file type: {ext}", style=Pack(margin=10))
        except Exception as e:
            self.preview_scroll.content = toga.Label(f"Error rendering preview: {str(e)}", style=Pack(margin=10))
